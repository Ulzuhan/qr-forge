...
const BASE = process.env.BASE ?? "http://127.0.0.1:3994";
const PUERTO_IDP = Number(process.env.PUERTO_IDP ?? 9998);
const ORIGEN_IDP = `http://127.0.0.1:${PUERTO_IDP}`;
const EMISOR = `${ORIGEN_IDP}/application/o/qr-forge`;
const CLIENT_ID = process.env.CLIENT_ID ?? "qrforge-pruebas";
...
// QR-Forge no tiene un "quién soy": lo que hay es el listado de códigos, que
// exige sesión. Sirve igual y además comprueba algo que la gente usa.
const puede = async () => (await fetch(`${BASE}/api/qr`, { headers: { cookie: cabecera() } })).status;
