...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  // guarda. Se responde 200 igualmente —el aviso era válido— sin tocar nada.
  if (aviso.sub) {
    const persona = await db.select().from(users).where(eq(users.oidcSub, aviso.sub)).get();
    if (persona) await db.delete(sessions).where(eq(sessions.userId, persona.id));
  }
