...
// Los datos ya no salen de una consulta en el servidor: los trae la API, que es
// la MISMA que ya existía. El marcado no cambia ni una clase.
type QrWithCount = QrConCuenta;
...
export function QrList({ baseUrl }: { baseUrl: string }) {
  const [qrs, setQrs] = useState<QrWithCount[] | null>(null);
  ...
  if (error) {
    return (
      <div className="text-center py-20 border border-dashed border-border rounded-lg bg-card/30">
        ...
        <h2 className="text-xl font-semibold mb-2">Could not load your QR codes</h2>
