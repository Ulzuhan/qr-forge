...
import { verEstadisticas, type Estadisticas } from "../lib/api";
...
export function Detail({ id, baseUrl }: { id: string; baseUrl: string }) {
  ...
  useEffect(() => {
    verEstadisticas(id).then(setDatos, (e: Error) => setError(e.message));
  }, [id]);
  ...
  const { qr, total, daily, countries, recent } = datos;
  const isStatic = qr.type === "static";
  const isExpired = qr.expiresAt && new Date(qr.expiresAt) < new Date();
  // La serie ya viene acotada por el servidor a `dailyDays`. Aquí NO se vuelve a
  // recortar: el gráfico hacía `slice(-30)` sobre el histórico entero, o sea los
  // últimos 30 días CON ACTIVIDAD, y los rótulos de los extremos mostraban otro
  // rango. El periodo es uno y lo fija quien hace la consulta.
  const diasVentana = datos.dailyDays;
  const escaneosVentana = daily.reduce((suma, d) => suma + d.count, 0);
          ...
          {!isStatic ? (
            <>
              <div className="grid grid-cols-2 gap-4">
                ...
                <Stat label={`Days with scans (${diasVentana}d)`} value={daily.length} />
                {/* Numerador y periodo, el MISMO: escaneos de la ventana entre los
                    días de la ventana. Antes dividía el total histórico entre
                    los días con actividad, que no es un promedio de nada. */}
                <Stat
                  label={`Avg scans / day (${diasVentana}d)`}
                  value={(escaneosVentana / diasVentana).toFixed(1)}
                />
              </div>
